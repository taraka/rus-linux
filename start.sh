...
cp target/x86_64-unknown-linux-musl/release/{sh,ls,cat,init} "$ROOT"/bin/
#need to make a proper init at some point the shell will do for now
cp target/x86_64-unknown-linux-musl/release/init "$ROOT"/init
chmod a+x "$ROOT"/bin/*
...
# Let's Go!!!!
qemu-system-x86_64 -D qemu.log -nographic -no-reboot -m 256 -kernel $VMLINUX  -initrd $INITRD -append "panic=1 console=ttyS0 init=/bin/init"